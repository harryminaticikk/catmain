...
let channel = client.channels.cache.get("960609674384797816")
...
if(interaction.customId == "selam_ver") {
         ...
         if(id == interaction.user.id) return interaction.reply({content: "<:hayir:961626266371493908> Kendine selam mı veriyorsun?", ephemeral: true})
  ...
  if(name == "öneri_olumlu") {
            ...
            if(name == "öneri_ayarlar") {
                let yetkiliRol = "960253056644173884"
                let message = interaction.message
            ...
            if(name == "öneri_red") {
              ...
              if(name == "isim_kabul") {
                let isimYetkili = "960253071492005888"
...
client.on('messageCreate', async message => {
  // isim-değiştir sistemi
              if(!message.author.bot && message.content.toLowerCase().startsWith("!isim-değiştir")) {
                let logChannel = "972744034512687114"
                ...
                let roles = ["960873261829337118", "960873347577688084", "960873431648305172", "960873508232101918"]
                if(!roles.some(rol => message.member.roles.cache.has(rol))) return message.reply({content: `<:961626266371493908> Bu komutu kullanmak için yetkiniz bulunmamaktadır.`})
                if(bool) return message.reply({content: `<:hayir:961626266371493908> Şu anda bir isim değiştirme isteğiniz bulunmakta, daha sonra tekrar deneyiniz.`})
                if(!isim) return message.reply({content: `<:hayir:961626266371493908> Lütfen bir isim girip öyle deneyin!`})
                
                if(isim.match(/[^a-zA-Z0-9- ]/g)) return message.reply({content: `<:hayir961626266371493908> İsimde sayı ve harften başka karakter kullanılamaz!`})
                
                if(isim.length > 22) return message.reply({content: `<:hayir:961626266371493908> İsim en fazla 22 karakter olabilir!`})
                message.reply({content: `<:evet:961626266467987456> İsminiz **\`${isim}\`** olarak değiştirilecektir, yetkililerimiz ilgileniyor.`})
                ...
                .setFooter({text: `miamidev.xyz`, iconURL: message.guild.iconURL()})
                .setTimestamp()
  ...
  // Öneri sistemi
  if(message.channel.id == "965655954890850365" && !message.author.bot) {
                ...
                .setColor("BLUE")
                .setAuthor({name: `${message.author.username} Bir öneri gönderdi`, iconURL: message.author.avatarURL()})
                ...
                .then(msg => {
                    ...
                    .on('collect', async (i) => {
                        if(i.customId === "slm_"+ message.id) {
                            ...
                            if(i.user.id == message.author.id) return i.reply({content: `<:hayir:961626266371493908> Kendine selam veremezsin dostum, hadi ama!`, ephemeral: true})
                            if(users.includes(i.user.id)) return i.reply({content: `<:hayir:961626266371493908> Zaten selam verdin!`, ephemeral: true})
                            await i.deferUpdate();
                            c++;
                            users.push(i.user.id)
                            message.reply({content: `**${i.user.username}**: Aleyküm selam, hoş geldin!`})
                            .catch(() => { 
                                msg.edit({content: `<:hayir:961626266371493908> Bu mesaj bulunamadı.`, components: [new discord.MessageActionRow().addComponents([btn.setDisabled(true)])]})
                            })
